...
func _process(delta):
	...
	if spawn_timer <= 0.0:
		...
		spawn_timer = max(4, 7.0 - max(0, waves_count/10.0)) + randf_range(0, 3)
		var randCount = randf_range(0, max(0, waves_count/10.0))
		print(spawn_timer)
		randCount = floor(randCount)
		print(randCount)
		for i in (1 + randCount):
			var offset = Vector2(200 * randf_range(-1,1), 20 * randf_range(-1,1))
